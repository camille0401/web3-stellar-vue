    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6">
      <div class="relative px-8 py-12 md:py-20 rounded-[3rem] overflow-hidden">
        ...
        <div class="max-w-3xl mx-auto text-center">
          <div>
            <div class="inline-flex font-medium bg-clip-text text-transparent bg-linear-to-r from-purple-500 to-purple-200 pb-3">{{$t("changelog.cta03_tips")}}</div>
          </div>
          <h2 class="h2 bg-clip-text text-transparent bg-linear-to-r from-slate-200/60 via-slate-200 to-slate-200/60 pb-4">{{$t("changelog.cta03_title")}}</h2>
          <p class="text-lg text-slate-400 mb-8">{{$t("changelog.cta03_desc")}}</p>
          <form class="inline-flex max-w-sm w-full">
            <div class="w-full flex flex-col sm:flex-row justify-center max-w-xs mx-auto sm:max-w-none">
              <input type="email" class="form-input py-1.5 w-full mb-3 sm:mb-0 sm:mr-2 rounded-full bg-slate-800/30 border-slate-700" :placeholder="$t('changelog.cta03_input_placeholder')" :aria-label="$t('changelog.cta03_input_placeholder')" />
              <button class="btn text-slate-900 bg-linear-to-r from-white/80 via-white to-white/80 hover:bg-white transition duration-150 ease-in-out group" type="submit">{{$t('changelog.cta03_button')}} <span class="tracking-normal text-purple-500 group-hover:translate-x-0.5 transition-transform duration-150 ease-in-out ml-1">-&gt;</span></button>
            </div>
          </form>
        </div>
      </div>
